 ...
 function loadDoc(filepath, elid) {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      // '<tr><td class="padd">'+'200s'+'</td><td class="padd">'+data['200s'][0]+'</td></tr>'+
...
function loadDocTest(filepath, elid) {
 var xhttp = new XMLHttpRequest();
 xhttp.onreadystatechange = function() {
   if (this.readyState == 4 && this.status == 200) {
     var data = JSON.parse(this.responseText)
...
function loadStats(elid){
  filepath = '../data/ODI-'+elid+'-Highlights.json'
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      var data = JSON.parse(this.responseText)
...
function loadStatsTest(elid){
  filepath = '../data/Test-'+elid+'-Highlights.json'
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      var data = JSON.parse(this.responseText)
      ...
      '<tr><td>'+'50s'+'</td><td>'+data['50s'][0]+'</td></tr>'+
      '<tr><td>'+'100s'+'</td><td>'+data['100s'][0]+'</td></tr>'+
      '<tr><td>'+'200s'+'</td><td>'+data['200s'][0]+'</td></tr>'+
      '<tr><td>'+'Not Outs'+'</td><td>'+data['Not Outs'][0]+'</td></tr>'+
      '<tr><td>'+'Ducks'+'</td><td>'+data['Ducks'][0]+'</td></tr>'+
      '</tbody></table></div>';
    }
